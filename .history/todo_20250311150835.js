...
async function updateTodoState(todoId, newState) {
  try {
    const response = await fetch(`http://your-api-url/todos/${todoId}`, {
      method: "PATCH",
      headers: {
    ...
    console.log(`Todo ${todoId} state updated to ${newState}`);
  } catch (error) {
