...
app.get("/api/todos", async (req, res) => {
  try {
    ...
    // const getTodostodos = await Todo.find().sort({ updatedAt: -1 });
...
app.put("/api/todos/:id", async (req, res) => {
  try {
    ...
    const getTodos = await Todo.find().sort({ updatedAt: -1 });
    console.log("updatedtodo:", getTodos);
    res.status(200).json(getTodos);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
//     const allowedFields = ["title", "category", "state"]; // Allowed fields to update

//     // Filter req.body to only allow specific fields
//     const updateData = {};
...
//     // Fetch updated todos from the database after update
//     const updatedTodos = await Todo.find().sort({ updatedAt: -1 });
