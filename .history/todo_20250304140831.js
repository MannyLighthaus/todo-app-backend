...
app.post("/api/todos", async (req, res) => {
  try {
    console.log("Incoming Request Body:", req.body); // Log the request body to check if it's correctly formatted.
    const newTodoItem = await Todo.create(req.body); // Save to MongoDB
    console.log("Saved Todo:", newTodoItem);
